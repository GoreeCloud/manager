...
| PF-012 | Complete GoreeCloud Policy decision/evidence integration beyond source-contract adoption, including live caller identity, authenticated decision exchange where applicable, policy distribution, provenance/freshness/expiry semantics, enforcement-coordination boundaries, obligations handling, target-environment validation, and production acceptance without transferring Policy authority to Manager. | P0 | Policy v1 source request construction and decision-evidence validation are implemented. Live runtime/distribution/enforcement/production acceptance remains migration-required. |
